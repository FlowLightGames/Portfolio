...
func set_power(p: int):
	...
	r_top.cast_to=Vector2(0,(8+(power-1)*16)-1)
	r_bottom.cast_to=Vector2(0,(8+(power-1)*16)-1)
	r_left.cast_to=Vector2(0,(8+(power-1)*16)-1)
	r_right.cast_to=Vector2(0,(8+(power-1)*16)-1)
...
func explode():
	if(!exploded):
		...
		vert.disabled=false
		vert.shape.extents.x=(used_rect.size.x/2)*16-1
	...
	#	print ("vert"+str(vert.shape.extents))
		...
		hori.disabled=false
		hori.shape.extents.y=(used_rect.size.y/2)*16-1
...
#	ani.material.set_shader_param("Type",color)
	exp_tile.material.set_shader_param("Type",color)
