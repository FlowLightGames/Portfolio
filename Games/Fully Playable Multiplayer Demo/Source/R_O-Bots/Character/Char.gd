...
export(int, 5) var body_type
export(int, 31) var body_color

export(int, 21) var face_type
export(int, 31) var face_color
...
func place_bomb():
	if (Plc_check.get_overlapping_bodies().empty()&&(BOMB_USED<BOMB_NUM)):
		var tmp=CURR_BOMB.instance()
		...
		if(!Exp_check.get_overlapping_areas().empty()):
			tmp.explode()
...
func move_loop():
	move_and_slide(velocity*movspeed*calculate_boost())
...
func _set_shader():
	...
	Body.material.set_shader_param("Type",body_color)
	Face.material.set_shader_param("Type",face_color)
